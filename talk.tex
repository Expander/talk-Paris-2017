...
\begin{frame}{Debatable ways to calculate $\Delta M_h$}
  ...
  \begin{tabular}{lllll}
    ...
    \midrule
    EFT/mixed & $\Delta\lambda^{1L}$ & $\Delta\lambda^{2L}$ \\
    \midrule
    $Q_\text{match}$ & $[\MS/2, 2\MS]$ & $[\MS/2, 2\MS]$ \\
    % $Q_\pole$ & $[M_t/2, 2M_t]$ & $[M_t/2, 2M_t]$ \\
    % $y_t(M_Z)$ & 1L vs.\ 2L & 2L vs.\ 3L \\
    % $\alpha_s(M_Z)$ & 0L vs.\ 1L & 0L vs.\ 1L \\
    $\lambda(\MS)$ & $0$ vs.\ $\frac{v^2}{\MS^2}$ & $0$ vs.\ $\frac{v^2}{\MS^2}$ \\
    \midrule
    EFT/mixed & $\Delta M_h^{1L}$ & $\Delta M_h^{2L}$ \\
    \midrule
    $Q_\pole$ & $[M_t/2, 2M_t]$ & $[M_t/2, 2M_t]$ \\
    $y_t(M_Z)$ & 1L vs.\ 2L & 2L vs.\ 3L \\
    $\alpha_s(M_Z)$ & 0L vs.\ 1L & 1L vs.\ 2L \\
    \bottomrule
  \end{tabular}
\end{center}
\end{frame}
